...
coef_matrix <- coef_table %>%
  ...
  pivot_wider(.,
              names_from = supp_naics,
              values_from = aij,
              values_fill = 0) %>%
  arrange(y, client_naics)

distance_table <- coef_matrix %>%
  pivot_longer(-c(y,client_naics),
               names_to = "supp_naics",
               values_to = "aij") %>%
  group_by(y,client_naics) %>%
  mutate(D = rank(aij)) %>%
...
distance_table_reg <- distance_table %>%
  ...
  select(y,client_naics,supp_naics) %>%
  group_by(y,client_naics) %>%
...
controls_table_reg <- distance_table_reg %>%
  left_join(.,agg) %>%
  select(y,client_naics,Nr,GProf,roe,pretret_earnat,npm) %>%
  pivot_wider(id_cols = c(y,client_naics),
              names_from = Nr,
...
write.csv(dat_final,"export/dat_final_clients.csv",row.names = F, na = "")